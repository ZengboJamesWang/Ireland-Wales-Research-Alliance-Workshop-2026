                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filteredSubmissions.map((s) => (
                      <tr key={s.id} className="hover:bg-white/[0.02] transition-colors"><td className="px-8 py-6"><p className="text-white font-bold">{s.first_name} {s.lastName}</p><p className="text-slate-500 text-xs">{s.email}</p></td><td className="px-8 py-6 text-slate-300 text-sm">{s.institution}</td><td className="px-8 py-6">{s.is_presenting ? <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-black rounded-lg border border-emerald-500/20 uppercase tracking-wider">AUTHOR</span> : <span className="px-3 py-1 bg-slate-800 text-slate-500 text-[10px] font-black rounded-lg uppercase tracking-wider">GUEST</span>}</td><td className="px-8 py-6 max-w-sm">{s.is_presenting ? <details className="cursor-pointer group/details"><summary className="text-sm text-emerald-400 font-bold list-none hover:text-emerald-300 transition-colors">READ ABSTRACT</summary><div className="mt-4 p-5 bg-slate-950 rounded-2xl border border-white/10 text-xs text-slate-400 leading-relaxed shadow-xl"><p className="text-white font-black text-sm mb-3 border-b border-white/5 pb-2">{s.title}</p><p className="whitespace-pre-wrap">{s.abstract}</p></div></details> : <span className="text-slate-700 italic text-xs">N/A</span>}</td><td className="px-8 py-6 text-right text-slate-600 text-[10px] font-black uppercase">{new Date(s.created_at).toLocaleDateString('en-GB')}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
